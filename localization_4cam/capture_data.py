...
def main():
    print(f"Starting {NUM_CAMERAS} threaded cameras at {CAM_WIDTH}x{CAM_HEIGHT}... Please wait.")
    cameras = [ThreadedCamera(i) for i in range(NUM_CAMERAS)]
    ...
    while True:
        ...
        for i, cam in enumerate(cameras):
            ...
            # 2. Add visual UI text to the frame
            if recording_wand:
                cv.putText(frame, "REC WAND", (10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            cv.putText(frame, f"Cam {i}", (10, 60), cv.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        ...
        # 3. Create a 2x2 visual grid (Top: Cam0, Cam1 | Bottom: Cam2, Cam3)
