...
class Cameras:
    def __init__(self):
        ...
        # Initialize Standard USB Webcams using OpenCV
        self.num_cameras = len(self.camera_params)
        print(f"Initializing {self.num_cameras} cameras...")
        
        self.caps = []
        for i in range(self.num_cameras):
            # Change 'i' to specific indices if your OS assigns webcams randomly (e.g., 0, 2, 4)
            cap = cv.VideoCapture(i)
            cap.set(cv.CAP_PROP_FRAME_WIDTH, 640)
            cap.set(cv.CAP_PROP_FRAME_HEIGHT, 480)
            
            # Attempt to disable Auto-Exposure & Auto-White Balance (Support varies by webcam)
    ...
    def edit_settings(self, exposure, gain):
        # Update settings for standard USB Webcams
    ...
    def _camera_read(self):
        ...
        for cap in self.caps:
            ret, frame = cap.read()
            if not ret:
                # If frame drops, append a blank frame to maintain array dimensions
        ...
        for i in range(0, self.num_cameras):
            ...
            # Smooth out color noise for better HSV tracking
            frames[i] = cv.GaussianBlur(frames[i], (5, 5), 0)
    ...
    def _find_dot(self, img):
        # Convert BGR (OpenCV default) to HSV for color isolation
        hsv = cv.cvtColor(img, cv.COLOR_BGR2HSV)
        
        # Tuned values for Green LED tracking
        lower_green = np.array([33, 46, 200])
        upper_green = np.array([78, 255, 255])
        
        # Apply Color Thresholding
        mask = cv.inRange(hsv, lower_green, upper_green)
        
        # Fix white core (Morphological Closing)
        ...
        for contour in contours:
            # Area constraint: ignore tiny specks of noise
            if cv.contourArea(contour) > 20:
                ...
                if moments["m00"] != 0:
                    ...
                    cv.circle(img, (center_x,center_y), 3, (0,0,255), -1)
...
# --- All Following Global Functions Remain Unchanged ---
...
def make_square(img):
    ...
    for i in range(feather_pixels):
        ...
        new_img[ay - i - 1, :] = img[0, :] * (1 - alpha)
        new_img[ay + img.shape[0] + i, :] = img[-1, :] * (1 - alpha)
